...
class IterativeRefinementGenerator(object):
    ...
    def generate(self, models, sample, prefix_tokens=None):
        ...
        for step in range(self.max_iter + 1):
            ...
            decoder_out, pos_distrib = model.forward_decoder(
                prev_decoder_out, encoder_out, target=sample['target'][0], **decoder_options
            )

            imgname = "pos_origin/pos_distrib" + str(sample['id'][0].cpu().detach().numpy()) + '.jpg'
            plt.imsave(imgname, pos_distrib.cpu().detach().numpy())
