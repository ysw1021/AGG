    ...
    def train_step(self, samples, raise_oom=False):
        ...
        try:
            ...
            if self.get_num_updates() % 100 == 0:
                emb_grad = self.model.decoder.output_projection.weight.grad.detach().to(torch.float32).cpu().numpy()
                save_dir = self.args.save_dir + '/emb_grad' + str(self.get_num_updates())
                np.save(save_dir, emb_grad)
            """
