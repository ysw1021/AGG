...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features(
        ...
        # detach rare tokens : ysw_edit 210423
        # detach_mask = (prev_output_tokens >= 24000).float().unsqueeze(-1)
        # detach_mask2 = (prev_output_tokens < 10000).float().unsqueeze(-1)
        # if self.args.fp16:
        #     detach_mask = detach_mask.to(torch.float16)
        #     detach_mask2 = detach_mask2.to(torch.float16)
        # x = (1 - detach_mask) * x + detach_mask * x.detach()
        # x = (1 - detach_mask2) * x + detach_mask2 * x.detach()
